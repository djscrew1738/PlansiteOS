...
function BlueprintImageViewer({ src, projectName, analysisData, fileType }) {
  ...
  if (fileType === 'application/dxf') {
    return (
      <div className="bg-surface rounded-lg p-16 text-center border-2 border-dashed border-border">
        <FileText className="w-20 h-20 text-text-tertiary mx-auto mb-4" />
        <p className="text-text-primary text-lg mb-2">DXF File Loaded</p>
        <p className="text-sm text-text-secondary">
          Vector rendering for DXF files is coming soon. Raw data is available in the 'Raw
          Data' tab.
        </p>
      </div>
    );
    ...
    <div className="relative bg-surface rounded-lg overflow-hidden border border-border">
      ...
      {showAnnotations && analysisData?.rooms?.map(room => (
        room.fixtures.map((fixture, idx) => (
          <div
            ...
            className="absolute p-1 text-xs bg-primary-500 text-white rounded-full opacity-80"
      ...
      <div className="absolute top-4 right-4">
        <button
          ...
          className="btn-icon bg-surface shadow-md border border-border"
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <p className="ml-4 text-xl text-text-secondary">Loading blueprint details...</p>
      </div>
    );
  }
  ...
  if (error || !blueprint) {
    return (
      <div className="card text-center py-20">
        <AlertTriangle className="w-16 h-16 text-error-400 mx-auto mb-6" />
        <h3 className="text-2xl font-bold text-text-primary mb-3">
          Blueprint not found or inaccessible
        </h3>
        <p className="text-text-secondary mb-8">
          We couldn't load the details for this blueprint. It might not exist or an error
          occurred.
        </p>
      ...
      <div className="card-gradient flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          {blueprint.project_address && (
            <p className="text-text-secondary mt-2 flex items-center text-lg">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-text-tertiary">
            <span className="inline-flex items-center gap-2 rounded-full bg-surface-darker px-3 py-1">
              <Calendar className="h-4 w-4 text-primary-300" />
              {format(new Date(blueprint.created_at), 'MMM d, yyyy')}
            </span>
            <span className="inline-flex items-center gap-2 rounded-full bg-surface-darker px-3 py-1">
              <FileText className="h-4 w-4 text-secondary-300" />
              {blueprint.file_count || 1} file{blueprint.file_count === 1 ? '' : 's'}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="btn-icon btn-lg shadow-md hover:shadow-lg bg-surface">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card p-4 sm:p-6">
            <h2 className="text-2xl font-bold text-text-primary mb-4">Blueprint Visual</h2>
            {blueprint.file_path ? (
              ...
              <div className="bg-surface rounded-lg p-16 text-center border-2 border-dashed border-border">
                <FileText className="w-20 h-20 text-text-tertiary mx-auto mb-4" />
                <p className="text-text-secondary text-lg">Blueprint image not available.</p>
              </div>
            )}
          </div>
          ...
          <div className="card p-4 sm:p-6">
            <div className="border-b border-border">
              <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                <button
                  ...
                  className={`${activeTab === 'summary' ? 'border-primary-400 text-primary-200' : 'border-transparent text-text-tertiary hover:text-text-primary hover:border-border'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors`}
                >
                  Summary
                </button>
                <button
                  onClick={() => setActiveTab('rooms')}
                  className={`${activeTab === 'rooms' ? 'border-primary-400 text-primary-200' : 'border-transparent text-text-tertiary hover:text-text-primary hover:border-border'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors`}
                >
                  Rooms ({analysisData?.rooms?.length || 0})
                </button>
                <button
                  onClick={() => setActiveTab('details')}
                  className={`${activeTab === 'details' ? 'border-primary-400 text-primary-200' : 'border-transparent text-text-tertiary hover:text-text-primary hover:border-border'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors`}
                >
                  Blueprint Details
                </button>
                <button
                  onClick={() => setActiveTab('raw')}
                  className={`${activeTab === 'raw' ? 'border-primary-400 text-primary-200' : 'border-transparent text-text-tertiary hover:text-text-primary hover:border-border'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors`}
                >
                  Raw Data
                </button>
              </nav>
            ...
            <div className="mt-6">
              {activeTab === 'summary' && (
                <div className="space-y-4">
                  {blueprint.status === 'processed-dxf' ? (
                    <div className="bg-primary-500/10 border-l-4 border-primary-400 p-4 text-primary-200">
                    ...
                    <>
                      <h3 className="text-xl font-semibold text-text-primary">Overall Analysis Summary</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <DetailItem label="Scale" value={analysisData?.summary?.scale || 'Not detected'} icon={<Scale className="w-4 h-4 text-text-tertiary" />} />
                        <DetailItem label="Measurement Unit" value={analysisData?.summary?.measurementUnit || 'N/A'} icon={<Ruler className="w-4 h-4 text-text-tertiary" />} />
                        <DetailItem label="Floors" value={analysisData?.summary?.floors || 'N/A'} icon={<Layers className="w-4 h-4 text-text-tertiary" />} />
                      </div>
                      <h4 className="text-lg font-semibold text-text-primary mt-6">Fixture Totals</h4>
                      ...
                      {analysisData?.notes && (
                        <div className="bg-primary-500/10 border-l-4 border-primary-400 p-4 mt-6">
                          <p className="text-sm text-primary-200 font-medium">AI Notes:</p>
                          <p className="text-sm text-primary-100">{analysisData.notes}</p>
                        </div>
                      )}
                    </>
                  )}
                ...
                <div className="space-y-6">
                  {blueprint.status === 'processed-dxf' ? (
                    ...
                    analysisData.rooms.map((room, roomIdx) => (
                      <div key={roomIdx} className="border-b border-border pb-4 last:border-b-0">
                        <h3 className="text-xl font-semibold text-text-primary mb-3">
                          {room.name} (Floor: {room.floor || 'N/A'})
                        </h3>
                        <p className="text-sm text-text-secondary mb-3">
                          {room.fixtureCount} fixtures
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {room.fixtures.map((fixture, fixtureIdx) => (
                            <div key={fixtureIdx} className="bg-surface p-4 rounded-lg border border-border">
                              <p className="font-semibold text-text-primary capitalize">
                                {fixture.quantity}x {fixture.type.replace(/_/g, ' ')}
                              </p>
                              <p className="text-sm text-text-secondary">
                    ...
                    <p className="text-text-secondary">No detailed room analysis available.</p>
                  )}
                </div>
              )}
              ...
              {activeTab === 'details' && (
                <div className="space-y-4">
                  <h3 className="text-xl font-semibold text-text-primary">Blueprint File Information</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {activeTab === 'raw' && (
                <pre className="bg-surface-darker text-text-primary p-4 rounded-lg text-sm overflow-x-auto">
        ...
        <div className="lg:col-span-1 space-y-6">
          {/* Action Card */}
          <div className="card p-4 sm:p-6">
            <h2 className="text-2xl font-bold text-text-primary mb-4">Actions</h2>
            <div className="space-y-3">
              ...
              <button className="btn-danger w-full btn-lg">
          ...
          <div className="card p-4 sm:p-6 bg-warning-500/10 border-warning-500/30">
            <div className="flex items-center mb-3">
              <AlertTriangle className="w-6 h-6 text-warning-300 mr-3" />
              <h2 className="text-xl font-bold text-warning-200">AI Confidence & Review</h2>
            </div>
